class decomposition():
    def decomposition(df, timeslice, location = None, returnDIC = None):
        ...
        # # Location
        # # # Current locations are Narragansett Bay specific 
        # # # and are used to determine mixed layer depth in m
        # # # For other locations, simply add another `elif` statement
        if location == 'PLT':
            MLD = 7
        elif location == 'GB':
            MLD = 9
        ...
        df['Alkalinity [umol/kg]'] = 477.62 + 51.99 * df['Salinity [PSU]']    # based on RML from Pimenta et al (2023) for Narr Bay
        ...
        if not 'avgWindSpeed' in MN.keys():
            print('Your dataframe does not include a column for wind speed labeled as "avgWindSpeed." Default wind speed is set to 5 m/s.')
            U10 = 5
        ...
        c0 = 3.7
        c1 = 1.165
        z = 2.4                                              # z (wind sensor height) based on Narr. Bay sensors
        U = U10
        A = a * k**2 * U **2
        B = g * z
        ...
        MN['H+ [umol/kg]'] = results['Hfree']           # H+ concentration [umol/kg]
        MN['DIC [umol/kg]'] = results['dic']            # DIC [umol/kg]
        MN['pCO2 [ppm]'] = results['pCO2']              # pCO2 [uatm]

        STD['H+ [umol/kg]'] = results['u_Hfree']        # H+ error [umol/kg] (from TA, pH, S, and T)
        STD['DIC [umol/kg]'] = results['u_dic']         # DIC error [umol/kg] (from TA, pH, S, and T)
        STD['pCO2'] = results['u_pCO2']                 # pCO2 error [uatm] (from TA, pH, S, and T)
        # -------------------------------------------------------------------------------------------------
        ...
        # SOLVE CARBONATE SYSTEM (daily timescales for ASF)------------------------------------------------
        # # Rather than use daily/weekly/monthly means for solve for ASF, we use finest temporal scale available
        # # to solve at EVERY time step before taking a time-mean.
        pH = df['pH final']                             # pH
        S = df['Salinity [PSU]']                        # Salinity
        T = df['Temperature [degC]']                    # Temperature  
        TA = df['Alkalinity [umol/kg]']                 # Alkalinity

        results = pyco2.sys(par1 = TA, par2 = pH, par1_type = 1, par2_type = 3, temperature = T, salinity = S)
        
        df['pCO2 [ppm]'] = results['pCO2']              # pCO2 [uatm]
        ...
        # molC/m2/yr
        fgco2_ann = decomposition.co2flux(df['pCO2 [ppm]'], 410, df['Temperature [degC]'], df['Salinity [PSU]'], U10)
        ...
        # umolC/kg/month
        df['DIC_flux [umolC/kg]'] = -(fgco2_month/MLD)*(1e6)/rho
        MN2 = df.resample(timeslice, on='DateTime').mean()           # MN contains daily/monthly means
        MN['DIC_flux [umolC/kg]'] = MN2['DIC_flux [umolC/kg]']
        # -------------------------------------------------------------------------------------------------
    
        # *************************************************************************************************

        # DIC bio------------------------------------------------------------------------------------------
        # # DIC bio is the residual of DIC after accounting for mixing and ASF
        MN['∆DIC_bio [umolC/kg]'] = MN['∆DIC'] - MN['DIC_flux [umolC/kg]'] - MN['∆DIC_mix']
        # -------------------------------------------------------------------------------------------------
        ...
        MN['∆TA_bio [umolC/kg]']=MN['∆DIC_bio [umolC/kg]']*(-16/107)
        # -------------------------------------------------------------------------------------------------
        ...
        # Based on the relationship presented in Pimenta et al (2023)
        # 477.62 + 51.99 * S = TA
        MN['∆TA_mix [umolC/kg]'] = (MN['Salinity [PSU]'] * 51.59).diff()
        # -------------------------------------------------------------------------------------------------
        ...
        alk_mix = MN['∂H/∂TA'] * MN['∆TA_mix [umolC/kg]']       # TA mixing: ∂H/∂TA * ∆TAmix
        alk_bio = MN['∂H/∂TA'] * MN['∆TA_bio [umolC/kg]']       # TA bio: ∂H/∂TA * ∆TAbio
        dic_bio = MN['∂H/∂DIC'] * MN['∆DIC_bio [umolC/kg]']     # DIC bio: ∂H/∂DIC * ∆DICbio
        ASF = MN['∂H/∂DIC'] * MN['DIC_flux [umolC/kg]']        # DIC air sea flux: ∂H/∂DIC * ∆DICflux
        ...
        # *************************************************************************************************
        # # If returnDIC == True, function will return ∆DIC, ∆S, etc. values and counts
        # # which includes ASF
        # # Otherwise, the dH components are returned
        if returnDIC == True:
            return MN, STD, count
        else:
